...
Для запуска большинства тестов в IntelliJ IDEA должен быть установлен плагин: DB Browser(Заходим в Settings и там в плагинах находим Database Navigator).
...
Для это закомментировать код для боевой базы, строки с 28 по 52 и раскомментировать строки тестовой базы строки 57 по 82 (Сталинград),
87 по 111 (Севастополь) или 115 по 139 (Курск);
2. Изменить настройки для запуска тестов в классе TestBase (src/test/java/baseForTests/TestBase.java)
для этого закомментировать код для боевого сайта строка 104 и раскомментировать строки тестового сайта строка 107 (Сталинград),
110 (Севастополь) или 113 (Курск);
...
Классы CertificateTest и NameNecklacesTest отключены, так как данный функционал пока не работает на сайте.
