...
И уже в нем нужно проверить настройки подлючения к Базе данных.
...
1. Изменить настройки подлючения к базе данных в классе DBWorker путь D:\test-repo\src\main\java\sql
...
Посмотрев какие тесты провалились нужно прогнать их повторно вручную, если они продолжают падать и после посторного прогона(за исключением классов указанных выше) нужно уже смотреть каждый индивидульно включив функцию открытия браузера(раскомментировать строку кода в классе аннотация @BeforeEach, строка  options.setHeadless(true);) и во время прогона уже смотреть в браузере, что именно сломалось.
Если на прогноне все еще непонятно, что происходит, то нужно вручную пройти все шаги теста непосредственно на сайте.
